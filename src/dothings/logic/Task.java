...
public class Task implements Comparable<Task> {
	 ...
	 * 
	 * @param desc
	 *            the description of the new task object.
	 */
	 ...
	 *            the description of the new task object.
	 * @param start
	 *            the start time of the new task object.
	 * @param end
	 *            the end time of the new task object.
	 * @param name
	 *            the name of the new task object.
	 */
	 ...
	 *            the name of the new task object.
	 * @param stat
	 *            the status of the task, whether it is marked as complete or not.
	 */
	public Task(String desc, DateTime start, DateTime end, String name,
	            boolean stat) {
		startDateTime = start;
	 ...
	 * 
	 * @param line
	 *            a string input to be separated.
	 * @return a new task object with the specified fields.
	 */
	public static Task parseTaskFromString(String line) {
		String[] tokens = line.split(DELIMITER);
		DateTime start = (tokens[0].equals(NULL_START)) ? null : new DateTime(
		        tokens[0]);
		DateTime end = (tokens[1].equals(NULL_END)) ? null : new DateTime(
		        tokens[1]);
	...
	public String toString() {
		String start = (startDateTime == null) ? NULL_START : startDateTime
		        .toString();
		String end = (endDateTime == null) ? NULL_END : endDateTime.toString();
		String taskAlias = (alias == null) ? NULL_ALIAS : alias;
		
		return start + DELIMITER + end + DELIMITER + taskAlias + DELIMITER
		        + status + DELIMITER + description;
	}
	...
	public String getDateTimeString() {
		if (startDateTime != null && endDateTime != null) {
			return String.format(DATE_DISPLAY_FORMAT_1,
			        getDateString(startDateTime), getTimeString(startDateTime),
			        getDateString(endDateTime), getTimeString(endDateTime));
		} else if (startDateTime != null) {
			return String.format(DATE_DISPLAY_FORMAT_2,
			        getDateString(startDateTime), getTimeString(startDateTime));
		} else if (endDateTime != null) {
			return String.format(DATE_DISPLAY_FORMAT_2,
			        getDateString(endDateTime), getTimeString(endDateTime));
	...
	public static String getTimeString(DateTime date) {
		...
		String time = df.format(date.getHourOfDay()) + ":"
		        + df.format(date.getMinuteOfHour());
		return time;
	}
	...
	public static boolean isAliasValid(String alias) {
		...
		if (alias.equalsIgnoreCase("completed")
		        || alias.equalsIgnoreCase("all")) {
			return false;
		}
	 ...
	 * 
	 * @param alias
	 *            the string from which the task index is retrieved.
