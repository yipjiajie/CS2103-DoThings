...
public class DateParser {
	...
	/**
	 * Get the date format type of the input. returns 0 if date contains day, month and year returns
	 * 1 if date contains day and month only returns 2 if date is written in English (Tuesday, wed
	 * etc) returns 3 if date is yesterday/today/tomorrow returns -1 if invalid
	 * 
	 * @param input
	...
	private static boolean isFormat3(String input) {
		return input.equalsIgnoreCase(DATE_TODAY)
		        || input.equalsIgnoreCase(DATE_TOMORROW)
	...
	/**
	 * Parse the input string with date format type 0 and returns a DateTime object with the
	 * corresponding date
	 * 
	 * @param input
	...
	/**
	 * Parse the input string with date format type 1 and returns a DateTime object with the
	 * corresponding date
	 * 
	 * @param input
	 * @return a DateTime object with the date set to the input date
	...
	private static DateTime parseDateFormat1(String input, DateTime date) {
		...
		if (targetMonth < currentMonth
			...
			date = new DateTime(date.getYear() + 1, targetMonth, targetDay, 0,
			        0);
		} else {
	...
	/**
	 * Parse the input string with date format type 2 and returns a DateTime object with the
	 * corresponding date
	 * 
	 * @param input
	 * @return a DateTime object with the date set to the input date
	...
	private static DateTime parseDateFormat2(String input, DateTime date) {
		DateTime tempDate = dateFormats.get(FORMAT_TYPE_2_INDEX - 1)
		        .parseDateTime(input);
	...
	/**
	 * Parse the input string with date format type 3 and returns a DateTime object with the
	 * corresponding date
	 * 
	 * @param input
	 * @return a DateTime object with the date set to the input date
	...
	private static int[] splitDayMonth(String date) {
		...
		for (int i = 0; i < tokens.length; i++) {
			try {
				...
				DateTime tempDate = DateTimeFormat.forPattern("MMM")
				        .parseDateTime(tokens[i]);
