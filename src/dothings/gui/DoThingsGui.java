...
public class DoThingsGui extends JFrame {
	 ...
	 * Basically it mimics a user command to list incomplete tasks
	 * 
	 * It calls the same function which is entered when the ENTER key is keyed. 
	 * However the feedback has to be overwritten. 
	 */
	...
	/**
	 * creates task panel scroll JScrollPane which sits below the 
	 * input field (i.e. white box). Gives scroll
	 * property to panel within it.
	 */
	...
	/**
	 * Creates task panel JPanel. It is inserted to taskPanelScroll.
	 */
	...
	/**
	 * creates text panel to sit behind the input field JPanel.
	 */
	private void createTextPanel() {
		textPanel = new JPanel();
		textPanel.setFocusable(false);
		textPanel.setBackground(Color.white);
		textPanel.setBounds(TEXT_PANEL_X_OFFSET, TEXT_PANEL_Y_OFFSET,
		        TEXT_PANEL_WIDTH, TEXT_PANEL_HEIGHT);
		contentPane.add(textPanel);
	}
	
	/**
	 * creates title label JLabel
	 */
	private void createTitleLabel() {
		...
		headingLabel.setBorder(new EmptyBorder(HEADING_LABEL_BUFFER,
		        HEADING_LABEL_BUFFER, HEADING_LABEL_BUFFER,
		        HEADING_LABEL_BUFFER));
		headingLabel.setOpaque(true);
		headingLabel.setBackground(Color.white);
		headingLabel.setFont(new Font(PLUTO_COND_LIGHT, Font.PLAIN,
		        HEADING_LABEL_FONT_SIZE));
		headingLabel.setHorizontalAlignment(SwingConstants.CENTER);
		headingLabel.setBounds(HEADING_LABEL_X_OFFSET, HEADING_LABEL_Y_OFFSET,
		        HEADING_LABEL_WIDTH, HEADING_LABEL_HEIGHT);
		contentPane.add(headingLabel);
	}
	
	/**
	 * creates feedback label JLabel which sits below the title. 
	 * Feedback for users are printed here.
	 */
	private void createFeedbackLabel() {
		feedbackLabel = new JLabel();
		feedbackLabel.setFocusable(false);
		feedbackLabel.setForeground(Color.GRAY);
		feedbackLabel.setFont(new Font(PLUTO_EXLIGHT, Font.PLAIN,
		        FEEDBACK_FONT_SIZE));
		feedbackLabel.setHorizontalAlignment(SwingConstants.CENTER);
		feedbackLabel.setBounds(FEEDBACK_X_OFFSET, FEEDBACK_Y_OFFSET,
		        FEEDBACK_WIDTH, FEEDBACK_HEIGHT);
		contentPane.add(feedbackLabel);
	}
	
	/**
	 * creates text input field JTextField where user types in commands
	 */
	private void createInputField() {
		inputField = new JTextField();
		inputField.setSelectedTextColor(HIGHLIGHT_FONT_DARK_BLUE);
		inputField.setSelectionColor(HIGHLIGHT_Yellow);
		inputField.setBounds(INPUT_FIELD_X_OFFSET, INPUT_FIELD_Y_OFFSET,
		        INPUT_FIELD_WIDTH, INPUT_FIELD_HEIGHT);
		inputField.setBorder(null);
		inputField.setBackground(INPUT_FIELD_BACKGROUND_LIGHT_BLUE);
		inputField.setForeground(Color.BLACK);
		inputField.setFont(new Font(PLUTO_EXLIGHT, Font.PLAIN,
	...
	private void createContentPane() {
		...
		setBounds(CONTENT_PANE_X_OFFSET, CONTENT_PANE_Y_OFFSET,
		...
		setShape(new RoundRectangle2D.Double(ZERO, ZERO, FRAME_WIDTH,
		        FRAME_HEIGHT, SHAPE_DIMENSION, SHAPE_DIMENSION));
		setIconImage(iconImage);
	}
	
	public static class ResponsiveContent {
		
		private static final Color MESSAGE_ELSE_TIME_BACKGROUND_GREEN = new Color(
		        153, 204, 102);
		private static final Color FONT_ELSE_TIME_WHITE = new Color(255, 255,
		        255);
		private static final Color MESSAGE_OVERDUE_BACKGROUND_RED = new Color(
		        204, 51, 61);
		private static final Color FONT_OVERDUE_WHITE = new Color(255, 255, 255);
		private static final Color MESSAGE_TODAY_BACKGROUND_YELLOW = new Color(
		        255, 255, 51);
		private static final Color FONT_TODAY_DARK_GREY = new Color(102, 102,
		        102);
		private static final Color FONT_FLOAT_WHITE = new Color(255, 255, 255);
		private static final Color MESSAGE_FLOAT_BACKGROUND_TURQUOISE = new Color(
		        153, 204, 153);
		private static final Color MESSAGE_MARKED_BACKGROUND_LIGHT_GREY = new Color(
		        204, 204, 204);
		...
		/**
		 * function to create task object panel which holds 
		 * task description, alias and date & time
		 * text areas
		 * 
		 * This function is called whenever a list of task is generated by the GUI. 
		 * 
		 ...
		 * @param i
		 */
		private static void createTaskObjects(int aliasExtension,
		                                      int descriptionExtension,
		                                      int change, int i) {
		 ...
		 * @param i
		 */
		private static void createTaskDescription(int descriptionExtension,
		                                          int i) {
			taskDescription.add(new JTextArea());
			taskDescription.get(i).setFont(
			        new Font(PLUTO_LIGHT_FONT, Font.PLAIN,
			                TASK_DESCRIPTION_FONT_SIZE));
			taskDescription.get(i).setBounds(TASK_DESCRIPTION_X_OFFSET,
			        TASK_DESCRIPTION_Y_OFFSET,
			        FRAME_WIDTH - TASK_DESCRIPTION_WIDTH,
			        TASK_DESCRIPTION_HEIGHT + descriptionExtension); 
		 ...
		 * @param i
		 */
		private static void createAliasField(int aliasExtension,
		                                     int descriptionExtension, int i) {
			alias.add(new JTextArea());
			alias.get(i).setFont(
			        new Font(PLUTO_COND_EXLIGHT_FONT, Font.PLAIN,
			                ALIAS_FONT_SIZE));
			alias.get(i).setBounds(ALIAS_X_OFFSET,
			        ALIAS_Y_OFFSET + descriptionExtension, ALIAS_WIDTH,
			        ALIAS_HEIGHT + aliasExtension);
		...
		private static void createDateTimeField(int descriptionExtension, int i) {
			...
			dateTime.get(i).setFont(
			        new Font(PLUTO_COND_EXLIGHT_FONT, Font.PLAIN,
			                DATE_TIME_FONT_SIZE));
			dateTime.get(i).setBounds(DATE_TIME_X_OFFSET,
			        DATE_TIME_Y_OFFSET + descriptionExtension, DATE_TIME_WIDTH,
			        DATE_TIME_HEIGHT);
		...
		/**
		 * Creates task object which is one panel holding 
		 * all the description, alias and date & time
		 * text fields
		 * 
		 * @param aliasExtension
		 * @param descriptionExtension
		 ...
		 * @param i
		 */
		private static void taskObjectPanel(int aliasExtension,
		                                    int descriptionExtension,
		                                    int change, int i) {
			taskObjectPanel.add(new JPanel());
			taskObjectPanel.get(i).setBounds(
			        ZERO,
			        ZERO + change,
			        FRAME_WIDTH,
			        MESSAGE_PANEL_HEIGHT + descriptionExtension
			                + aliasExtension);
		...
		/**
		 * function which sends userInput to MainLogic.runLogic(userInput)
		 * 
		 * It calls the MainLogic class to process the user's input.
		 * It also receives the return variable of the function 
		 * MainLogic.runLogic() and takes the strings placed in the return variable
		 * and append the various feedback information to the respective text areas
		 * and labels.
		 * 
		 * The information of the task is also stored in the return variable and is
		 * used by the GUI to determine which color scheme to apply for the tasks
				...
				if (feedbackType.equals(DEFAULT_HELP)) {
					...
					if (feedbackType.equals(ERROR_CODE)) {
						...
						for (int i = ZERO; i < numOfTask; i++) {
							...
							createTaskObjects(aliasOverflowExtension,
							        descriptionOverflowExtension, heightChange,
							        i);
							heightChange += descriptionOverflowExtension;
							heightChange += aliasOverflowExtension;
							
							if (taskStatus.get(i).equals(MARK_CODE)) {
								setTaskObjectColorScheme(i,
								        MESSAGE_MARKED_BACKGROUND_LIGHT_GREY,
								        FONT_MARKED_GREY);
							} else if (taskTime.get(i).equals(FLOATING)) {
								setTaskObjectColorScheme(i,
								        MESSAGE_FLOAT_BACKGROUND_TURQUOISE,
								        FONT_FLOAT_WHITE);
							} else if (taskTime.get(i).equals(DUE_TODAY)) {
								setTaskObjectColorScheme(i,
								        MESSAGE_TODAY_BACKGROUND_YELLOW,
								        FONT_TODAY_DARK_GREY);
							} else if (taskTime.get(i).equals(OVERDUE)) {
								setTaskObjectColorScheme(i,
								        MESSAGE_OVERDUE_BACKGROUND_RED,
								        FONT_OVERDUE_WHITE);
							} else {
								setTaskObjectColorScheme(i,
								        MESSAGE_ELSE_TIME_BACKGROUND_GREEN,
								        FONT_ELSE_TIME_WHITE);
							}
		...
		/**
		 * determines additional height that needs to be added to 
		 * the task object to fit text overflow in alias field
		 * 
		 * @param i
		...
		/**
		 * determines additional height that needs to be added to the 
		 * task object panel to fit text overflow in description field
		 * 
		 * @param i
		 * @return additionalHeight
		 */
		...
		private static void setTaskPanelHeight() {
			...
			taskPanel
			        .setPreferredSize(new Dimension(FRAME_WIDTH, heightChange));
		}
		...
		/**
		 * sets color schemes for message panel, 
		 * task description font, alias font and date time
		 * font
		 * 
		 * @param i
		 ...
		 * @param fontColor
		 */
		private static void setTaskObjectColorScheme(int i,
		...
		/**
		 * Sets error message into feedback field and 
		 * selects all text in input field
		 * 
		 * @param feedbackDesc
		 */
		...
		private static void createHelpTextarea(String desc) {
			help = new JTextArea();
			help.setFont(new Font(PLUTO_COND_EXLIGHT_FONT, Font.PLAIN,
			        HELP_FONT_SIZE));
			help.append(desc);
			help.setBounds(HELP_X_OFFSET, HELP_Y_OFFSET, HELP_WIDTH,
			        HELP_HEIGHT);
